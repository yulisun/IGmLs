function [labels] = solveEnergyMRF(f,D,sup_img,opt)
[h,w]   = size(sup_img);
nbr_sp  = max(sup_img(:));
...
for i = 1:sum(adj_mat(:))
    ...
    feature_x = f(index_node_x);
    feature_y = f(index_node_y);
    Dpq(i) = norm(feature_x-feature_y,2)^2;
    dist(i) = max(norm(location_center(index_node_x,:)-location_center(index_node_y,:),2),1);
end
sigma = mean(Dpq);
Vpq = exp(-Dpq/(2*sigma+eps));
Vpq = Vpq ./dist;
gama = opt.gama*sum(f)/sum(Vpq);
edgeWeights(:,3) = gama*Vpq;                  % node 1 ---> node 2
edgeWeights(:,4) = gama*Vpq;                  % node 2 ---> node 1
%% termWeights
opt.disth = opt.wh;
termWeights = zeros(nbr_sp,2);
fy = max(sqrt(sum(D.^2,2))-opt.disth,0);
alfa = opt.alfa*sum(f)/(sum(fy)+eps);
termWeights_tp = f + alfa*fy;
Normtp = remove_outlier(f);
lamda = graythresh(Normtp);
termWeights_sp = lamda;
...
addpath('GC');
[~, labels] = graphCutMex(termWeights, edgeWeights);
